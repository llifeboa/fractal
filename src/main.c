...
/*   Updated: 2019/10/15 08:10:06 by llifeboa         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int mouse_press(int button, int x, int y, void *param)
{
	...
	if (button == 4 || button == 5)
	{
		...
		sign = button == 4 ? 1 : -1;
		main_data->step = (main_data->y_max - main_data->y_min) / 100 * 4;
...
int main(int argc, char const *argv[])
{
	...
	main_data->command = "double	map(double current, double end, double new_start, double new_end) {	return (current / end * (new_end - new_start) + new_start); } __kernel void render(__global int *out, double x_min, double x_max, double y_min, double y_max) { int i = get_global_id(0) / 1920;  int j = get_global_id(0) % 1920;  double x_z = map(j, 1920, x_min, x_max);			double y_z = map(i, 1080, y_min, y_max);			double x = 0;			double y = 0;			int iter = 0;			double x_temp;			while (x*x + y*y <= 4 && iter < 256)			{				x_temp = x*x - y*y + x_z;				y = 2*x*y + y_z;				x = x_temp;				iter++;			}			if (iter == 256)			{				out[i * 1920 + j] = 0;			}			else			{				out[i * 1920 + j] = 0x010001 * iter;			} }";
